...
export default class MapGenerator {
    ...
    GenerateCaves() {
        ...
        for (let i = 0; i < caveSize; i++) {
            ...
            var y = Math.floor(random(this.startAreaSize, contextMap.tiles[0].length));
    ...
    GenerateGold() {
        ...
        for (let i = 0; i < caveSize; i++) {
            var x = Math.floor(random(this.startAreaSize * 2, contextMap.tiles.length));
            var y = Math.floor(random(this.startAreaSize * 2, contextMap.tiles[0].length));
            var tile = contextMap.tiles[x][y];
            var tilesSur = contextMap.getSurroundingTiles(tile);
            //First loop--------------------------------------
    ...
    generateLife() {
        ...
        for (let i = 0; i < tiles.length; i++) {
            for (let j = 0; j < tiles[0].length; j++) {
                if (tiles[i][j].frontTile == null && tiles[i][j].backTile == InnerStone) {
                    if (random(100) > 80) {
